...
class MainWindow(CTk):
    def __init__(self):
        ...
        self.speed_animate_menu = -10

        self.btn = CTkButton(self, text='▶️', command=self.toggle_show_menu, width=30)
        self.btn.place(x=0, y=0)

        # --- основна частина ---
        self.chat_field = CTkScrollableFrame(self,fg_color="#735184")
        self.chat_field.place(x=0, y=0)

        self.message_entry = CTkEntry(self, placeholder_text="Введіть повідомлення", height=40 )
        self.message_entry.place(x=0, y=0)

        self.send_button = CTkButton(self, text=">", width=50, height=40, command=self.send_message, font=(None, 20))
        self.send_button.place(x=0, y=0)

        self.open_img_button = CTkButton(self, text="📂", width=50, height=40, command=self.open_img)
        ...
        try:
            ...
            self.sock.connect(('0.tcp.ngrok.io', 19342))
            hello = f"TEXT@{self.username}@[SYSTEM] {self.username} приєднався(-лась) до чату!!\n"
            self.sock.send(hello.encode())
            threading.Thread(target=self.recv_message, daemon=True).start()
        except:
            self.add_message(f"Все поломалось телефонуйте фіксикам💥💥💥: {Exception}")
    ...
    def adaptive_ui(self):
        ...
        self.open_img_button.place(x=self.winfo_width() - self.open_img_button.winfo_width()*2,
                               y=self.winfo_height() - self.send_button.winfo_height())
        self.message_entry.place(x=self.menu_frame.winfo_width(), y=self.send_button.winfo_y())
        self.message_entry.configure( width=self.winfo_width() - self.menu_frame.winfo_width() - self.send_button.winfo_width()*2)
    ...
    def save_name(self):
        ...
        if new_name:
            ...
            self.add_message(f"Ваш нік успішно оновлено, тепер вас звати {self.username}")
        else:
            self.add_message(f"!!!!!Ви не ввели нове імя!!!")

    # --- Відправка повідомлень
    def send_message(self):
        message = self.message_entry.get().strip()
        if message:
            self.add_message(f"Ви:\n {message}")
            data = f"TEXT@{self.username}@{message}\n"
            try:
                self.sock.send(data.encode())
            except:
                self.add_message("🔴Трапилась помилка відправлення🔴")
    ...
    def open_img(self):
        ...
        try:
            ...
            self.add_message(f"Помилка відправки зображенння: {e}")
