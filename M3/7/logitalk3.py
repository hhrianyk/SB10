...
from socket import socket, AF_INET, SOCK_STREAM#new
...
class MainWindow(CTk):
   def __init__(self):
       ...
       self.open_img_button = CTkButton(self, text='📂', width=50, height=40, command=self.open_image)#new
       self.open_img_button.place(x=0, y=0)#new
       ...
       try:
           ...
           self.sock.connect(('6.tcp.ngrok.io', 18434))
   ...
   def adaptive_ui(self):
       ...
       self.open_img_button.place(x=self.winfo_width() - 105, y=self.send_button.winfo_y()) #new
   ...
   #1 показ повідомлення або зображення
   ...
   # 3 підключенння для отримання повідомлення
   def recv_message(self):
    ...
    #4 підключення для розбір протоколу повідомлень відображення повідомлення
   ...
   #5 підключення для відправки картинок
   def open_image(self):
